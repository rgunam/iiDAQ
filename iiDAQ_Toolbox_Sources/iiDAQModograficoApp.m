classdef iiDAQModograficoApp < handle
    %
    % Usage:
    %     app.iiDAQModograficoApp
    ...
    properties (Constant)
        AppPathBase = {'/iiDAQModografico'};
        AppClass = 'iiDAQModograficoApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = iiDAQModograficoApp()
        ...
        function startApp(obj)
            ...
            mlock;
            iiDAQModograficoApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'modoGrafico_iiDAQ')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong modoGrafico_iiDAQ
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'modoGrafico_iiDAQ').getString, ...
            ...
            if nargout(@modoGrafico_iiDAQ) == 0
                eval('modoGrafico_iiDAQ');
            else
                obj.AppHandle = eval('modoGrafico_iiDAQ');
            end
